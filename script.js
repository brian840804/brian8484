...
// === PATCH (2025-09-08): Click on Silk Road -> open/select event "西方食材進入中國" ===
(function () {
  try {
    var TARGET_NAME = '西方食材進入中國';
    ...
    // 尋找地圖上對應事件名稱的 marker layer
    function findEventMarkerLayerByName(name) {
      var m = getMap(); if (!m) return null;
      var layers = m._layers || {};
      // 先嘗試從 DOM label 配對，再回推 layer
      var icons = Array.from(document.querySelectorAll('.leaflet-marker-icon'));
      var iconEl = icons.find(function (el) {
      ...
      if (iconEl) {
        for (var k in layers) {
          var lyr = layers[k];
          if (lyr && lyr._icon === iconEl) return lyr;
        }
      }
      // 後備：直接在 layers 中找有 _icon 且 _icon 內含該文字的
      for (var key in layers) {
        var layer = layers[key];
        if (layer && layer._icon) {
          var labelEl = layer._icon.querySelector && layer._icon.querySelector('.marker-label');
    ...
    function triggerMarkerClickFor(name) {
      var m = getMap(); if (!m) return false;
      var lyr = findEventMarkerLayerByName(name);
      if (lyr) {
        // 先試著觸發 Leaflet 事件
        try { if (typeof lyr.fire === 'function') { lyr.fire('click'); } } catch (e) {}
        // 再直接點 DOM（以涵蓋原碼若在 DOM click 綁互動的情形）
        try { if (lyr._icon) { lyr._icon.click(); } } catch (e) {}
        return true;
      }
      return false;
    }

    // 建立可點擊的 hitbox（與既有 hover hitbox 無關，避免互相覆寫）
    var clickHitbox = null;
    function ensureClickHitbox() {
      var m = getMap(); if (!m) return null;
      if (!Array.isArray(silkRoadCoords)) return null;
      if (clickHitbox) return clickHitbox;
      clickHitbox = L.polyline(silkRoadCoords, {
        color: '#000000',
        opacity: 0.001,
        weight: 20,
        interactive: true
      });
      // 綁定 click：點擊絲路 => 觸發對應事件
      clickHitbox.on('click', function () {
        // 如果當下找不到 marker（例如時間軸篩掉），就暫時輪詢幾次以等待出現
        if (triggerMarkerClickFor(TARGET_NAME)) return;
        var tries = 0;
        var timer = setInterval(function () {
          tries++;
          if (triggerMarkerClickFor(TARGET_NAME) || tries > 20) {
            clearInterval(timer);
          }
        }, 300);
      });
    ...
    function tryAttach() {
      var m = getMap(); if (!m) return false;
      var hb = ensureClickHitbox(); if (!hb) return false;
    ...
    var timer = setInterval(function () {
      ...
      if (tryAttach() || attempts > 40) {
        clearInterval(timer);
        if (attempts <= 40) {
          console.log('✅ 丝路點擊已綁定至事件「' + TARGET_NAME + '」');
        } else {
          console.log('ℹ️ 未能建立絲路點擊綁定（地圖或座標尚未就緒）');
        }
      }
    }, 500);
    ...
    console.warn('PATCH: 點擊絲路對應事件失敗', e);
  }
})();
// === END PATCH ===


// === PATCH (2025-09-08): Clicking Silk Road focuses on event "西方食材進入中國" ===
(function () {
  try {
    const TARGET_NAME = '西方食材進入中國';
    ...
    // 嘗試找到對應的 marker，並模擬 click
    function triggerEventClick() {
      ...
      const icon = icons.find(el => {
        ...
        return label && (label.textContent || '').trim() === TARGET_NAME;
      });
      if (icon) {
    ...
    // 建立透明 hitbox，用於接收 click
    let clickbox = null;
    function ensureClickbox() {
      const m = getMap(); if (!m) return null;
      if (!Array.isArray(silkRoadCoords)) return null;
      if (clickbox) return clickbox;
      clickbox = L.polyline(silkRoadCoords, {
        color: '#000000',
        opacity: 0.001,
        weight: 18,
        interactive: true
      ...
      clickbox.on('click', () => {
        const ok = triggerEventClick();
        if (ok) {
          console.log('✅ 點擊絲路 → 已觸發「西方食材進入中國」事件');
        } else {
          console.log('ℹ️ 點擊絲路 → 找不到「西方食材進入中國」事件');
        }
      });
      return clickbox;
    }
    ...
    function tryAttach() {
      const m = getMap(); if (!m) return false;
      const cb = ensureClickbox(); if (!cb) return false;
      if (!m.hasLayer(cb)) cb.addTo(m);
      if (cb.bringToFront) cb.bringToFront();
      return true;
    }
    ...
    const timer = setInterval(() => {
      ...
      if (tryAttach() || tries > 40) {
        clearInterval(timer);
        if (tries <= 40) {
          console.log('✅ 已為絲路建立點擊觸發「西方食材進入中國」');
        } else {
          console.log('ℹ️ 未能建立絲路點擊觸發：地圖或座標尚未就緒');
        }
      }
    }, 500);
    ...
    console.warn('PATCH: 點擊絲路觸發事件失敗', e);
  }
})();
// === END PATCH ===
